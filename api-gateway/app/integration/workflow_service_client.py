"""
Workflow Service Client for API Gateway Integration
Provides client functionality to communicate with Workflow Service from API Gateway
"""
...
class WorkflowServiceClient:
    ...
    async def execute_workflow(self, workflow_data: Dict[str, Any], user_id: str) -> Dict[str, Any]:
        """
        Execute a workflow via the Workflow Service
        ...
        try:
            async with config.get_workflow_service_client() as client:
                payload = {
                    "workflow_data": workflow_data,
                    "user_id": user_id
                }

                logger.info(f"Sending workflow execution request for user {user_id}")
